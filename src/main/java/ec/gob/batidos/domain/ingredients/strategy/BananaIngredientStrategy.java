...
public class BananaIngredientStrategy implements IngredientStrategy {
  ...
  public List<Ingredient> getIngredientsNeeded(int volume) {
    ...
    Ingredient ingredient = Inventory.getInstance().getByName(IngredientNamesEnum.BANANA.getName());

    double factor = (double) volume / BaseShakeVolumeEnum.DEFAULT.getVolume();
    double newQuantity = IngredientsBaseSizeEnum.BANANA.getBaseQuantity() * factor;
